...
with open(afolder + args.dataset_name + "_adj_list.bin", 'rb') as f:
...
with tf.Graph().as_default():
	...
	with sess.as_default():
		...
		def train_step(x_batch, y_batch, neighbour_x, arity):
			"""
		...
		for epoch in range(args.start_epoch, args.n_epochs):
			...
			for i in range(len(train)):
				for batch_num in range(n_batches_per_epoch[i]):
					...
					x_batch, y_batch, neighbour_x= Batch_Loader_GNN(train[i], values_indexes, roles_indexes, role_val, args.batch_size, arity, whole_train[i],val_corres_sampled_dict)
					# print(x_batch.shape)
					# print(y_batch.shape)
					# print(neighbour_x.shape)
					tmp_loss = train_step(x_batch, y_batch, neighbour_x, arity)
					train_loss = train_loss + tmp_loss
				
			logger.info("nepoch: "+str(epoch+1)+", trainloss: "+str(train_loss))
			# print("nepoch: "+str(epoch+1)+", trainloss: "+str(train_loss))
			# exit()
