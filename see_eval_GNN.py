...
with open(afolder + FLAGS.dataset_name + "_adj_list.bin", 'rb') as f:
    val_corres_sampled_dict=pickle.load(f)
    ...
    def run(self):
        ...
        while True:
            ...
            if dat is None:
                ...
                (x_batch, y_batch, neighbour_x, arity, ind) = dat
                feed_dict = {
                aNaLP.input_x: x_batch,
                aNaLP.input_y: y_batch,
                aNaLP.arity: arity,
                aNaLP.neighbour_x: neighbour_x
                }
...
def eval_one(x_batch, y_batch, evaluation_queue, result_queue, data_index, pred_ind=0):
    ...
    for i in range(len(x_batch)):
        ...
        for tmpIndex in range(nn - 1):
            neighbour_x=get_neighbours(new_x_batch[listIndexes[tmpIndex]:listIndexes[tmpIndex + 1]],val_corres_sampled_dict)
            evaluation_queue.put((new_x_batch[listIndexes[tmpIndex]:listIndexes[tmpIndex + 1]], new_y_batch[listIndexes[tmpIndex]:listIndexes[tmpIndex + 1]], neighbour_x, arity, tmpIndex))
        neighbour_x=get_neighbours(new_x_batch[listIndexes[-1]:],val_corres_sampled_dict)
        evaluation_queue.put((new_x_batch[listIndexes[-1]:], new_y_batch[listIndexes[-1]:], neighbour_x, arity, nn-1))
